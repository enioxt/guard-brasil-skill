...
- **Qualidade NOME (caveat honesto):** recall do GLiNER em threshold 0.3 é borderline — entre runs, às vezes pega "Ana Carolina de Souza", às vezes não (scores 0.3–0.46). Mecanismo provado; recall não é confiável → revisão humana de nomes OBRIGATÓRIA (já no GUIA) + tunar threshold/labels é next-step real.
