...
import type { PIIPatternDefinition } from '../lib/pii-scanner.js';
import type { CustomPIIPattern } from '../pii-patterns.js';
...
interface HitlResult {
  ...
  fullText: string;
  shouldMatch: boolean;
  reviewerDecision: 'confirmed' | 'rejected' | 'partial' | 'skipped';
  note?: string;
}

function patternToDefinition(p: CustomPIIPattern): PIIPatternDefinition {
  return {
...
function highlight(text: string, matched: string): string {
...
async function run() {
  ...
  for (const entry of PCMG_CORPUS) {
    ...
    for (const finding of relevant) {
      ...
      const decision: HitlResult['reviewerDecision'] =
        answer === 's' ? 'confirmed'
        : answer === 'p' ? 'partial'
        : answer === 'n' ? 'rejected'
        : 'skipped';

      if (decision === 'rejected' && isExpected) falsePositiveCount++;
