...
const DEFAULT_DEDUCTIONS = { info: 2, warning: 5, error: 15, critical: 30 };
...
function collectPatternViolations(text, patterns, build, skip) {
    const violations = [];
    for (const pattern of patterns.map(clonePattern)) {
        let match;
        while ((match = pattern.exec(text)) !== null)
            if (!skip?.(match.index, match[0]))
                violations.push(build(match[0]));
    }
    return violations;
}
function detectInventedAcronyms(text, knownAcronyms, acronymDefinitionPattern) {
    const violations = [];
    ...
    while ((match = pattern.exec(text)) !== null) {
        ...
        if (!knownAcronyms.has(acronym) && !acronymDefinitionPattern.test(before))
            violations.push({ level: 'warning', category: 'invented_acronym', message: `Possible invented acronym: "${acronym}"`, matched: acronym });
    }
    return violations;
}
export function createAtrianValidator(config = {}) {
    ...
    function validateResponse(text) {
        ...
        for (const entity of blockedEntities)
            for (const matched of text.match(new RegExp(escapeRegex(entity), 'gi')) ?? [])
                violations.push({ level: 'critical', category: 'blocked_entity', message: `Blocked entity mentioned: "${entity}"`, matched });
    ...
    function validateAndReport(text) {
        const result = validateResponse(text);
        if (result.violations.length > 0)
            config.onViolation?.(result, text);
        return result;
    }
    function filterChunk(chunk) { return criticalBlocklistRegex ? chunk.replace(criticalBlocklistRegex, '***') : chunk; }
    return { validateResponse, validateAndReport, filterChunk };
}
//# sourceMappingURL=atrian.js.map