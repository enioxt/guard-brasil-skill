...
const cloneRe = (re) => new RegExp(re.source, re.flags.includes('g') ? re.flags : re.flags + 'g');
...
export function applyNERRules(text) {
    const findings = [];
    ...
    while ((mC = reC.exec(text)) !== null) {
        ...
        if (words.length < 2 || words.some(w => NER_C_STOP.has(w)))
            continue;
        findings.push({
            category: 'name',
...
//# sourceMappingURL=ner-rules.js.map