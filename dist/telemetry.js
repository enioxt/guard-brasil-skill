 ...
 * Record an API error
 */
export async function recordApiError(error, meta = {}) {
    if (!loadSupabase())
        return;
    try {
        await supabase
 ...
 */
export async function getRecentEvents(limit = 50, tenantId) {
    if (!loadSupabase())
        return [];
    try {
        let query = supabase
