...
function runBenchmark() {
    ...
    for (const [pattern, r] of results.entries()) {
        ...
        console.log(`${pattern.padEnd(17)}| ${(precision * 100).toFixed(1).padStart(8)}% | ${(recall * 100).toFixed(1).padStart(5)}% | ${(f1 * 100).toFixed(1).padStart(5)}% | ${String(r.tp).padStart(2)} | ${String(r.fp).padStart(2)} | ${String(r.fn).padStart(2)}`);
    }
