 ...
 */
import { createAtrianValidator, maskPublicOutput, buildLGPDDisclosure, createEvidenceChain, formatEvidenceBlock, buildAuditFields, sha256Text, sourceFingerprint, } from './lib/index.js';
import {} from './pii-patterns.js';
...
function buildInspectionReceipt(text, output, evidenceChain, provenance) {
    const inspectedAt = new Date().toISOString();
    const inputHash = sha256Text(text);
    const outputHash = sha256Text(output);
    let provenanceLevel = 'inspection_only';
    let source;
...
export class GuardBrasil {
    ...
    inspect(text, options = {}) {
        ...
        if (!atrianResult.passed) {
            ...
            if (errors.length > 0)
                issues.push(`ATRiAN: ${errors.length} violation(s) (${errors.map(v => v.category).join(', ')})`);
        }
 ...
 */
export function createGuardBrasil(config = {}) {
